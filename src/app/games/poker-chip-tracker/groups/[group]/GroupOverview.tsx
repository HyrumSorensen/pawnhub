...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog"; // Make sure you have this
...
const [memberTransactions, setMemberTransactions] = useState<MemberTransactionsMap>({});
const colorPalette = [
  "#4e79a7", "#f28e2c", "#e15759", "#76b7b2", "#59a14f",
  "#edc948", "#b07aa1", "#ff9da7", "#9c755f", "#bab0ac",
  "#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3",
  "#fdb462", "#b3de69", "#fccde5", "#d9d9d9", "#bc80bd",
  "#ccebc5", "#ffed6f", "#1f77b4", "#ff7f0e", "#2ca02c",
...
type MemberTransactionsMap = Record<
  string,
  Doc<"chipTransactions">[]
>;
...
useEffect(() => {
  const fetchTransactions = async () => {
    ...
    for (const member of members) {
      const res = await convex.query(api.pokerChipTracker.listChipTransactions, {
  ...
  if (members.length && chipTypes.length && Object.keys(memberTransactions).length === 0) {
    fetchTransactions();
  }
  ...
  members.forEach((member) => {
    ...
    transactions
      ...
      .forEach((tx) => {
        ...
        dailyTotals[date] = (dailyTotals[date] ?? 0) + tx.amount * value;
  ...
  return (
    <div className="mt-12">
      <h3 className="text-lg font-medium mb-4">Group Chip Value History</h3>
      <Line
        data={{
          labels: sortedDates,
          datasets: perUserCumulative.map((userData, index) => ({
            ...userData,
  ...
  <DialogContent className="max-w-3xl">
    ...
    {selectedUserTransactions && chipTypes.length ? (() => {
      ...
      <p className="text-sm text-muted-foreground">Loading or no transactions found.</p>
    )}
  </DialogContent>
</Dialog>
